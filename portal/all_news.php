...
session_start();
 ...
 // ---------- FOR RECENTS AND POPOLAR NEWS------------------------
    ...
    $query = $conn->prepare("SELECT * FROM s_categories");
    $query->execute();
    $category = array();
    while ($cat = $query->fetch(PDO::FETCH_ASSOC)) 
    {
        $category[] = $cat['categories_name'];
    }
    $totalCats = count($category);
    // ---------- FOR EVENT MARKEE------------------------
...
<header id="header" class="container">
            ...
            <ul class="nav" style="float: left;margin-left: 34%">
                ...
                <li class="dropdown"><a href="##">Categories</a>
                    ...
                    <?php for ($i=0; $i < $totalCats; $i++) { ?>
                        <li><a href="category.php?cat=<?php echo $category[$i]; ?>"><?php echo $category[$i]; ?></a></li>
                    <?php } ?>
                    </ul>
                </li>
    ...
    <div id="content" class="container" style="padding: 13px 40px 40px 40px;">
        ...
        <div id="main" class="row-fluid">
              ...
              <div style="background-color: white; margin-top: 2em; padding: 5px 5px 5px 5px;">
                <article class="post">
                    ...
                    <div class="entry-content">
                        <div class="row-fluid">
                            <div class="span4">
                                <a href="<?php echo $sources[$i]; ?>" title="<?php echo $headlines[$i]; ?>" rel="bookmark">
                                <img width="774" height="320" src="<?php echo $sources[$i]; ?>" alt="" />
                                </a>
                            </div>
                            <div style="display: inline-block; width: 85%;margin-top: 10px;">
            ...
            <div class="pagination magz-pagination" style="background-color: white;"><span class='page-numbers current' ><a   href="<?php if ($sum < $totalIds) {$next= $sum+10; echo "all_news.php?p=$next" ;} if ($sum == $totalIds) { echo '#!';} ?>"><?php if ($sum < $totalIds) { echo "load more";} if ($sum == $totalIds) { echo "No More to load";} ?></a></span>
            </div>
        ...
        <div id="sidebar" class="span4">
            ...
            <div id="tab1" style="display: block; background-color: white;">
                ...
                <div class="holder clearfix" style="height: 40px;"></div>
                <div class="clear"></div>
